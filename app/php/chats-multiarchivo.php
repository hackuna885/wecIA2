...
// ========== COMPATIBILIDAD: Soportar AMBOS formatos ==========

// CASO 1: Formato nuevo - múltiples archivos con 'documentos[]'
if (isset($_FILES['documentos']) && is_array($_FILES['documentos']['name'])) {
    
    $totalArchivos = count($_FILES['documentos']['name']);
    
    // Validar que no se excedan los 5 archivos
    if ($totalArchivos > 5) {
        http_response_code(400);
    ...
    for ($i = 0; $i < $totalArchivos; $i++) {
        ...
        if ($_FILES['documentos']['error'][$i] !== UPLOAD_ERR_OK) {
            ...
            switch ($_FILES['documentos']['error'][$i]) {
                case UPLOAD_ERR_INI_SIZE:
                    $errorMessage = 'El archivo excede la directiva upload_max_filesize en php.ini.';
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    $errorMessage = 'El archivo excede la directiva MAX_FILE_SIZE.';
                    break;
            ...
            http_response_code(500);
            echo json_encode(['error' => 'Error al subir el archivo ' . ($_FILES['documentos']['name'][$i]) . ': ' . $errorMessage]);
            error_log("Error de subida de archivo: " . $errorMessage . " (Código: " . $_FILES['documentos']['error'][$i] . ")");
            exit;
        }
        ...
        if (!in_array($mime_type, $tiposPermitidos)) {
            ...
            http_response_code(400);
            echo json_encode(['error' => 'Tipo de archivo no permitido: ' . $_FILES['documentos']['name'][$i] . '. Formatos aceptados: JPG, JPEG, PNG, PDF, DOCX, DOC, TXT, XLSX, XLS, CSV, PPTX, PPT.']);
        ...
        if ($_FILES['documentos']['size'][$i] > $tamañoMaximo) {
            ...
            http_response_code(400);
            echo json_encode(['error' => 'El archivo ' . $_FILES['documentos']['name'][$i] . ' excede el tamaño máximo de 10MB.']);
            exit;
        }
        ...
        try {
            ...
            if (!move_uploaded_file($_FILES['documentos']['tmp_name'][$i], $documentoTemporal)) {
                throw new Exception('No se pudo mover el archivo subido al directorio temporal.');
            }
            
            $archivosTemporal[] = $documentoTemporal;
...
// CASO 2: Formato antiguo - un solo archivo con 'documento'
elseif (isset($_FILES['documento']) && $_FILES['documento']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if ($_FILES['documento']['error'] !== UPLOAD_ERR_OK) {
        ...
        switch ($_FILES['documento']['error']) {
            case UPLOAD_ERR_INI_SIZE:
                ...
                break;
            case UPLOAD_ERR_FORM_SIZE:
                $errorMessage = 'El archivo subido excede la directiva MAX_FILE_SIZE.';
                break;
            case UPLOAD_ERR_PARTIAL:
                $errorMessage = 'El archivo se subió solo parcialmente.';
                break;
    ...
    if (!in_array($mime_type, $tiposPermitidos)) {
        http_response_code(400);
        echo json_encode(['error' => 'Tipo de archivo no permitido. Formatos aceptados: JPG, JPEG, PNG, PDF, DOCX, DOC, TXT, XLSX, XLS, CSV, PPTX, PPT.']);
        error_log("Intento de subida de archivo con tipo MIME no permitido: " . $mime_type);
        exit;
    }
